...
int fila_insere(Fila *fila, Aluno *aluno) {
  ...
  if (fila->tamanho == fila->capacidade_maxima) {
    int nova_capacidade = 2 * fila->capacidade_maxima;
    fila->fila_alunos = (Aluno *)realloc(fila->fila_alunos, nova_capacidade);
    fila->capacidade_maxima = nova_capacidade;
  }
...
Aluno *fila_busca(Fila *fila, int matricula) {
  if (fila != NULL && fila->tamanho > 0) {
    ...
    for (int i = 0; i < fila->tamanho; i++) {
      alu_acessa((&(fila->fila_alunos))[i], &matricula_aux, nome, curso);
