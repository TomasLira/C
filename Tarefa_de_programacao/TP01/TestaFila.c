...
float teste_fila_cria_com_dados_validos() {
  ...
  Fila *fila = fila_cria();
  if (fila != NULL) {
    printf("[Passou 1.00] - Teste da função fila_cria() com dados válidos!\n");
    pontuacao = 1.0;
  } else {
    printf("[Falhou 0.00] - Teste da função fila_cria() com dados válidos!\n");
  }
  return pontuacao;
}
...
float teste_fila_vazia_com_dados_validos() {
  ...
  if (fila != NULL && fila_vazia(fila)) {
    ...
    printf("[Falhou 0.00] - Teste da função fila_vazia() com dados válidos!\n");
  }
  return pontuacao;
}
